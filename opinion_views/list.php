...
// Aktive/beendete und archivierte Umfragen trennen
$active_polls   = array_filter($all_polls, fn($p) => $p['status'] !== 'archived');
$archived_polls = array_filter($all_polls, fn($p) => $p['status'] === 'archived');

/**
 * Gibt eine einzelne Poll-Karte aus.
 * Verwendet Variablen aus dem umgebenden Scope: $pdo, $current_user, $_tab_public_url, $_tab_process_url
 */
function _render_poll_card($poll, $current_user, $pdo, $tab_public_url, $tab_process_url) {
    ...
    $is_archived  = ($poll['status'] === 'archived');
        ...
        <div style="display: flex; justify-content: space-between; align-items: start; gap: 10px;">
            <div style="flex: 1; min-width: 0;">
                ...
                <div class="poll-meta">
                    ...
                    <?php else: ?>
                        <span class="poll-status" style="background:#888;color:white;">Archiviert</span>
                    <?php endif; ?>
                ...
                <?php
                // Zugangslink (nur für Ersteller und Admins, nicht bei archivierten)
                if (!$is_archived && ($is_creator || $is_admin)):
            ...
            <div style="display: flex; flex-direction: column; gap: 8px; flex-shrink: 0;">
                ...
                <?php if (!$is_archived && ($is_creator || $is_admin)): ?>
                    <form method="POST" action="<?php echo htmlspecialchars($tab_process_url); ?>" style="margin: 0;"
                          onsubmit="return confirm('Umfrage archivieren? Sie verschwindet aus der aktiven Liste.');">
                        <input type="hidden" name="action"   value="archive_poll">
                        <input type="hidden" name="poll_id"  value="<?php echo $poll['poll_id']; ?>">
                        <?php if (!empty($_SESSION['opinion_mtool_share_url'])): ?>
                            <input type="hidden" name="redirect_to" value="<?php echo htmlspecialchars($_SESSION['opinion_mtool_share_url']); ?>">
                        <?php endif; ?>
                        <?php if (!empty($OPINION_MTOOL_MODE) && !empty($MNr)): ?>
                            <input type="hidden" name="mtool_mnr" value="<?php echo htmlspecialchars($MNr); ?>">
                        <?php endif; ?>
                        <button type="submit" style="width: 100%; padding: 6px 12px; background: #f0f0f0; border: 1px solid #ccc; border-radius: 6px; cursor: pointer; font-size: 13px; color: #555;">
                            📦 Archivieren
                        </button>
                    </form>
                <?php endif; ?>
...
if (empty($active_polls) && empty($archived_polls)): ?>
    <div class="opinion-card">
...
<?php else: ?>
    ...
    <?php if (empty($active_polls)): ?>
        <div class="opinion-card" style="color: #666;">
            <p>Keine aktiven oder beendeten Meinungsbilder vorhanden.</p>
        </div>
    <?php else: ?>
        <?php foreach ($active_polls as $poll):
            _render_poll_card($poll, $current_user, $pdo, $tab_public_url, $tab_process_url);
        endforeach; ?>
    <?php endif; ?>

    <?php if (!empty($archived_polls)): ?>
        <details style="margin-top: 20px;">
            <summary style="cursor: pointer; font-weight: bold; color: #666; padding: 12px 16px; background: #f5f5f5; border-radius: 6px; border: 1px solid #ddd; list-style: none; display: flex; justify-content: space-between;">
                <span>📦 Bereits erledigt (<?php echo count($archived_polls); ?>)</span>
