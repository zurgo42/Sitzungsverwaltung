...
?>
    <div class="agenda-item" id="top-<?php echo $item['item_id']; ?>" 
        ...
        <!-- NACHTRÄGLICHE ANMERKUNGEN (für alle Teilnehmer nach dem Protokoll) -->
        <?php
        ...
        <div style="margin-top: 15px; padding: 12px; background: #e8f5e9; border: 2px solid #4caf50; border-radius: 6px;">
            ...
            <?php if ($is_secretary): ?>
                <!-- Protokollant speichert über Hauptformular -->
                <div class="form-group">
            ...
            <?php else: ?>
                <!-- Andere Teilnehmer haben eigenes Formular -->
...
<?php
// --- Meinungsumfragen dieser Sitzung ---
if (file_exists(__DIR__ . '/opinion_functions.php')) {
    ...
    $stmt_op = $pdo->prepare("
        ...
        WHERE op.list_id = ?
          AND op.target_type = 'list'
          AND op.status != 'deleted'
        HAVING response_count > 0
        ORDER BY op.created_at DESC
    ");
    $stmt_op->execute([$current_meeting_id]);
    $meeting_polls = $stmt_op->fetchAll(PDO::FETCH_ASSOC);

    if (!empty($meeting_polls)):
        $current_member_id = $current_user['member_id'] ?? null;
?>
<div style="margin: 30px 0 0 0;">
    <h4 style="margin: 0 0 10px 0; color: #333;">📊 Meinungsumfragen dieser Sitzung</h4>
    <?php foreach ($meeting_polls as $poll):
        $has_voted = false;
        if ($current_member_id) {
            $stmt_voted = $pdo->prepare("SELECT 1 FROM svopinion_responses WHERE poll_id = ? AND member_id = ?");
            $stmt_voted->execute([$poll['poll_id'], $current_member_id]);
            $has_voted = (bool)$stmt_voted->fetchColumn();
        }
        $status_label = match($poll['status']) {
            'active'   => ['🟢 Aktiv',   '#e8f5e9', '#2e7d32'],
            'closed'   => ['🔴 Beendet', '#fce4ec', '#c62828'],
            'draft'    => ['⚪ Entwurf',  '#f5f5f5', '#616161'],
            default    => [$poll['status'], '#f5f5f5', '#616161'],
        };
    ?>
    <div style="margin-bottom: 12px; padding: 14px 16px; background: #f9f9f9; border: 1px solid #ddd; border-left: 4px solid #7b1fa2; border-radius: 6px; display: flex; align-items: center; gap: 16px; flex-wrap: wrap;">
        <div style="flex: 1; min-width: 180px;">
            <strong style="font-size: 14px;"><?= htmlspecialchars($poll['title']) ?></strong>
        </div>
        <div style="display: flex; align-items: center; gap: 12px; flex-shrink: 0;">
            <span style="font-size: 12px; padding: 2px 8px; border-radius: 12px; background: <?= $status_label[1] ?>; color: <?= $status_label[2] ?>; font-weight: 600;">
                <?= $status_label[0] ?>
            </span>
            <span style="font-size: 13px; color: #555;">
                <?= (int)$poll['response_count'] ?> Antwort<?= $poll['response_count'] != 1 ? 'en' : '' ?>
            </span>
            <?php if ($has_voted): ?>
                <span style="font-size: 12px; color: #2e7d32; font-weight: 600;">✅ Teilgenommen</span>
            <?php endif; ?>
            <a href="opinion_standalone.php?poll_id=<?= (int)$poll['poll_id'] ?>"
               style="font-size: 12px; padding: 4px 12px; background: #7b1fa2; color: white; border-radius: 4px; text-decoration: none; white-space: nowrap;">
                Ergebnisse
            </a>
        </div>
    </div>
